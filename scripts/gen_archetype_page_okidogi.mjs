...
const data = JSON.parse(await fs.readFile('tmp/okidogi_5deck_summary.json','utf8'));

const arche = {
  title: '夠讚狗（イイネイヌ）｜優勝/上位 Deck（5副平均）',
  slug: 'okidogi',
  pokecabook: 'https://pokecabook.com/archives/149284',
};

function esc(s){return String(s).replace(/[&<>"']/g, c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]));}

function pickCore(arr){
  return arr.filter(x=>x.present>=3).slice(0, 120);
}
...
<body>
  <div class="wrap">
    ...
    <div class="box">
      ...
      <ul>${deckLinks}</ul>
      <div class="hint">提示：夠讚狗呢套通常係「闘能量 + 稜鏡/傳承」嘅混能量配置；分歧位多數落喺支援者（手牌干擾/換位）同 1-of tech。</div>
    </div>
    ...
    <div class="box">
      <ul>
        <li>核心一定係：<b>イイネイヌ</b>（5/5 出現，通常 3–4）。</li>
        <li>固定配套幾乎係：<b>ソルロック / ルナトーン</b>（5/5 出現，各 2）。</li>
        <li>打點+節奏位通常係：<b>ファイトゴング</b>、<b>ポケパッド</b>、同大量 <b>リーリエの決心</b>。</li>
      </ul>
    </div>
  </div>
</body>
...
await fs.mkdir('ptcg/decks/okidogi', { recursive: true });
await fs.writeFile('ptcg/decks/okidogi/index.html', html, 'utf8');
console.log('wrote ptcg/decks/okidogi/index.html');
